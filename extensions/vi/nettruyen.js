function NetTruyen() {
    ...
    this.mangaDetailsParse = (doc) => {
        const parseStatus = (stat) => {
            ...
            if (ongoingWords.includes(stat.toLowerCase())) {
                return "ONGOING"
            }
            if (completedWords.includes(stat.toLowerCase())) {
                return "COMPLETED"
            }
            return "UNKNOWN";
        }
        ...
        let artist = author;
        let glist = [];
        info.querySelectorAll("li.kind p.col-xs-8 a").forEach(it => {
            glist.push(it.innerText)
        });
    ...
    this.chapterFromElement = (element) => {
        const parseDate = (dateString) => new Date("20" + dateString.split("/").reverse().join('/')).getTime();
